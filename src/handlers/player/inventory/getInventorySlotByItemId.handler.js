...
export const getInventorySlotByItemIdHandler = async (socket, packetData) => {
    try {
      const itemIds = packetData.itemId;
      ...
      try {
        for(const itemId of itemIds){
          for(let slotIdx=0; slotIdx<25; slotIdx++){
              const redisSlot = JSON.parse(redisSlots[slotIdx]);
              if(redisSlot.itemId === itemId){
                  slots.push(PAYLOAD_DATA.InventorySlot(slotIdx, itemId, redisSlot.stack));
                  break;
              }
          }
        }
      } catch (error) {
        socket.emit(new CustomError(ErrorCodes.HANDLER_ERROR, 'getInventorySlot 반복문 에러'));
      }

      try {
        socket.write(PACKET.S2CGetInventorySlotByItemId(slots));
      } catch (error) {
        socket.emit(new CustomError(ErrorCodes.INVALID_PACKET, 'S2CGetInventorySlotByItemId 패킷 생성 에러'));
      }
    } catch (error) {
      socket.emit('error', new CustomError(ErrorCodes.HANDLER_ERROR, 'getInventorySlotByItemId 에러'));
    }
  };